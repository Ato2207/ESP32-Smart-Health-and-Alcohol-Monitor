...
const auto kSamplingRate = pulseOxSensor.SAMPLING_RATE_400SPS;
...
PulseOxData measure_pulseOx() {
    PulseOxData result = {0, 0};

    auto sample = pulseOxSensor.readSample(1000);
                    ...
                    if (bpm > 50 && bpm < 250) {
                        ...
                        if (kEnableAveraging) {
                            ...
                            if (averager_bpm.count() >= kSampleThreshold) {
                                ...
                                result.bpm = average_bpm;
                                result.spo2 = average_spo2;
                            }
                        } else {
                            result.bpm = bpm;
                            result.spo2 = spo2;
